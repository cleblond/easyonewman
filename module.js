...
//        var javaparams = new Array();
        ...
        /*if (appletoptions) {
            easyonewmanoptions[easyonewmanoptions.length] = appletoptions;
        ...
        if (readonly) {
//            easyonewmanoptions[easyonewmanoptions.length] = "false";  ///crl changed depict to true
//	     javaparams.menubar = "false";	    
//            easyonewmanoptions[easyonewmanoptions.length + 1] = "false";  ///crl changed depict to true
//	    easyonewmanoptions[easyonewmanoptions.length] = Y.one(topnode+' input.mol').get('value');  ///crl changed depict to true
	   // easyonewmanoptions[easyonewmanoptions.length] = Y.one(topnode+' input.mol').get('value');  ///crl 
		...
		//console.log("in module.js pos0="+pos0);
...
//                Y.one(topnode+' input.mol').set('value', this.find_java_applet(name).molFile())
    ...
    show_error : function (Y, topnode) {
        var errormessage = '<span class ="javawarning">'
            +M.util.get_string('enablejava', 'qtype_easyonewman')+
            '</span>';
...
//	newApplet.setAttribute('codebase','../../../easyonewman');
...
M.qtype_easyonewman2={
    insert_structure_into_applet : function(Y, slot, moodleroot){
...
M.qtype_easyonewman.init_showmyresponse = function(Y, moodle_version, slot, moodleroot){
...
M.qtype_easyonewman.init_showcorrectanswer = function(Y, moodle_version, slot, moodleroot){
    var handleSuccess = function(o) {
